 ...
 */
async function executeWithTimeout(queryPromise, timeoutMs = 120000, errorMessage = 'Database query timed out') {
    ...
    if (isSupabaseConfigured() && userId && supabase) {
      ...
      return await deduplicatedRequest(cacheKey, async () => {
        ...
        const { data, error } = await executeWithTimeout(
          queryPromise,
          150000,
          `Favorites fetch timed out after 150 seconds for user ${userId}`
        );
...
export async function incrementThoughtViews(thoughtId) {
  ...
  try {
    ...
    if (isSupabaseConfigured() && supabase) {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('View increment timed out after 120 seconds')), 120000);
      });
...
export async function toggleThoughtLike(thoughtId, userId = null) {
  ...
  try {
    ...
    if (isSupabaseConfigured() && userId && supabase) {
      ...
      // Create a promise that rejects after a timeout
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Like toggle timed out after 150 seconds')), 150000);
      });
...
export async function incrementThoughtShares(thoughtId) {
  ...
  try {
    ...
    if (isSupabaseConfigured() && supabase) {
      ...
      // Create a promise that rejects after a timeout
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Share increment timed out after 120 seconds')), 120000);
      });
