...
export async function getCategories() {
  ...
  try {
    ...
    const { data, error } = await executeWithTimeout(
      queryPromise,
      90000,
      'Categories fetch timed out after 90 seconds'
    );
...
export async function getComments(thoughtId: string) {
  ...
  try {
    ...
    const { data, error } = await executeWithTimeout(
      queryPromise,
      90000,
      `Comments fetch for thought ${thoughtId} timed out after 90 seconds`
    );
...
export async function addComment(thoughtId: string, userId: string, content: string, parentId?: string) {
  ...
  try {
    ...
    const { data, error } = await executeWithTimeout(
      queryPromise,
      120000,
      `Comment creation for thought ${thoughtId} timed out after 120 seconds`
    );
...
export async function createNotification(userId: string, type: string, title: string, message: string, data: any = {}) {
  ...
  try {
    ...
    const { data: notificationData, error } = await executeWithTimeout(
      queryPromise,
      90000,
      `Notification creation for user ${userId} timed out after 90 seconds`
    );
